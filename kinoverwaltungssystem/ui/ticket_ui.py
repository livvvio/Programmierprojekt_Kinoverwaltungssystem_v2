...
class TicketPerson:
    ...
    def calculate_price(self, is_late: bool, is_weekend: bool) -> tuple[float, list[str]]:
        """Berechnet den Ticketpreis inkl. Rabatten und gibt eine Beschreibungsliste zurück."""
...
def _do_pdf_download(movie, persons: list[TicketPerson], total: float,
                     show_hour: int, is_weekend: bool, order_id: str) -> None:
    """Generiert das PDF-Ticket und sendet es als Download an den Browser."""
    try:
...
class TicketUI:
    ...
    def render(self) -> None:
        """Rendert die Ticketkauf-Seite mit Film-, Vorstellungs- und Personenauswahl."""
        ui.query('body').style(
        ...
        def _get_total() -> float:
            return sum(p.calculate_price(_hour() >= 22, weekend_toggle.value)[0] for p in self.persons)

        def refresh_movie_info() -> None:
            """Aktualisiert die Film-Info-Karte bei Filmwechsel."""
                ...
                with ui.row().style('gap: 16px; align-items: center; width: 100%;'):
                    ...
                    with ui.element('div').style('flex: 1; min-width: 0;'):
                        ...
                        with ui.row().style('gap: 8px; align-items: center; flex-wrap: wrap; margin-bottom: 6px;'):
                            ui.label(movie_obj.genre.value).style('color: #4ade80; font-size: 12px; font-weight: 600;')
                        ...
                        if movie_obj.beschreibung:
                            desc = (movie_obj.beschreibung[:120] + '…') if len(
                                movie_obj.beschreibung) > 120 else movie_obj.beschreibung
                            ui.label(desc).style('color: #666; font-size: 11px; line-height: 1.5;')

        def refresh_summary() -> None:
            """Aktualisiert die Bestellzusammenfassung (Personen, Preise, Total)."""
            ...
            with summary_container:
                ...
                with ui.row().style(
                    ...
                    with ui.element('div'):
                        ...
                        ui.label(f'{hour:02d}:00').style('color: white; font-size: 16px; font-weight: 800;')
                        if is_late:
                            ui.label('Spätvorstellung').style('color: #a78bfa; font-size: 10px;')
                    with ui.element('div'):
                        ui.label('TAG').style('color: #555; font-size: 9px; font-weight: 700; letter-spacing: 1.5px;')
                ...
                if not self.persons:
                    with ui.element('div').style('text-align: center; padding: 20px 0; color: #444; font-size: 13px;'):
                    ...
                    with ui.element('div').style(
                        ...
                        for i, person in enumerate(self.persons):
                            ...
                            with ui.element('div').style(
                                    'background: #212121; border-radius: 8px; padding: 10px 12px; margin-bottom: 8px;'
                            ):
                                with ui.row().style(
                                        'justify-content: space-between; align-items: center; width: 100%;'):
                                    ui.label(person.name).style('color: white; font-weight: 600; font-size: 13px;')
                                ...
                                for d in (disc or [f'Grundpreis: {TICKET_GRUNDGEBUEHR:.2f} CHF']):
                                    ui.label(f'› {d}').style('color: #555; font-size: 10px; margin-top: 2px;')

                    total = sum(p.calculate_price(is_late, is_wknd)[0] for p in self.persons)
                    ui.element('div').style('height: 1px; background: #2a2a2a; margin: 12px 0;')
                    with ui.row().style(
                            'justify-content: space-between; align-items: baseline; width: 100%; margin-bottom: 14px;'
                    ):
                        with ui.element('div'):
                            ui.label('TOTAL').style(
                                'color: #666; font-size: 10px; font-weight: 700; letter-spacing: 2px;')
                            ui.label(f'{len(self.persons)} Person(en)').style('color: #555; font-size: 11px;')
                        with ui.row().style('align-items: baseline; gap: 4px;'):
                            ui.label(f'{total:.2f}').style('color: #e50914; font-size: 28px; font-weight: 900;')
                            ui.label('CHF').style('color: #e50914; font-size: 14px; font-weight: 700;')

                    with ui.row().style('gap: 8px; width: 100%;'):
                        ui.button('Abbrechen', on_click=lambda: _cancel_order()).props('no-caps unelevated').style(
                            'flex: 1; background: #242424 !important; color: #aaa; '
                            'font-weight: 600; border: 1px solid #333; border-radius: 6px; font-size: 13px; padding: 8px 0;'
                        )
                        ui.button('Kaufen', on_click=lambda: _checkout()).props('no-caps unelevated').style(
        ...
        def remove_person(idx: int) -> None:
            self.persons.pop(idx)
            refresh_summary()

        def add_person() -> None:
            """Validiert die Eingaben und fügt eine neue Person zur Bestellung hinzu."""
            add_error.set_text('')
        ...
        def _checkout() -> None:
            """Leitet zur Stripe-Zahlung weiter oder öffnet den Demo-Bestätigungsdialog."""
            if not self.persons:
        ...
        def _start_stripe_checkout(
            ...
            """Erstellt eine Stripe-Checkout-Session und leitet den Browser weiter."""
            try:
                import stripe
            except ImportError:
        ...
        def _show_confirmation_dialog(
            ...
            """Zeigt den Demo-Bestätigungsdialog mit PDF-Download-Option."""
            order_id = str(uuid.uuid4())[:8].upper()
            with ui.dialog() as dialog, ui.card().classes('rounded-xl').style(
                ...
                with ui.column().style('align-items: center; gap: 8px; padding: 24px 24px 16px;'):
                    ...
                    ui.label('Bestellung erfolgreich!').style('color: white; font-size: 22px; font-weight: 900;')
                ...
                with ui.element('div').style('padding: 16px 24px;'):
                    ...
                    with ui.row().style('justify-content: space-between; align-items: center; width: 100%;'):
                        ...
                        with ui.row().style('align-items: baseline; gap: 3px;'):
                            ui.label(f'{total:.2f}').style('color: #e50914; font-size: 20px; font-weight: 900;')
                ...
                with ui.row().style(
                    ...
                    with ui.element('div').style('flex: 1;'):
                        ui.label('PDF-Ticket herunterladen').style('color: white; font-size: 13px; font-weight: 700;')
                        ui.label('Dein Ticket als PDF speichern.').style('color: #555; font-size: 11px;')
                    ui.button('Download',
                              on_click=lambda _m=movie_obj, _p=persons_snap, _t=total, _h=hour, _w=is_wknd, _o=order_id:
                ...
                with ui.row().style(
                    ...
                    with ui.element('div'):
                        ui.label('Demo-Modus').style('color: #f59e0b; font-size: 11px; font-weight: 700;')
        ...
        with ui.element('div').style(
            ...
            with ui.element('div').style('flex: 1; display: flex; flex-direction: column; gap: 16px; min-width: 0;'):
                with ui.element('div').classes('section-card'):
                    ui.label('FILM').style(
                        'color: #555; font-size: 10px; font-weight: 700; letter-spacing: 2px; margin-bottom: 12px;'
                    )
                    selected_movie_id = ui.select(
                        options=movie_options, value=default_id,
                ...
                with ui.element('div').classes('section-card'):
                    ui.label('VORSTELLUNG').style(
                        'color: #555; font-size: 10px; font-weight: 700; letter-spacing: 2px; margin-bottom: 14px;'
                    )
                    with ui.row().style('gap: 16px; align-items: center; flex-wrap: wrap;'):
                        ...
                        with ui.element('div').style(
                            ...
                            with ui.element('div'):
                                ui.label('Wochenende').style('color: white; font-size: 13px; font-weight: 600;')
                                ui.label('Sa / So  +2.00 CHF').style('color: #555; font-size: 11px;')
                ...
                with ui.element('div').classes('section-card'):
                    ...
                    with ui.row().style('gap: 12px; align-items: flex-end; flex-wrap: wrap;'):
                        person_name = ui.input(label='Name').props('dark outlined').style('flex: 1; min-width: 140px;')
                        person_age = ui.number(label='Alter', value=25, min=0, max=120).props('dark outlined').style(
                            'width: 90px;')
                        with ui.element('div').style(
                                'display: flex; flex-direction: column; align-items: center; gap: 2px; padding-bottom: 4px;'
                        ):
                            ui.label('Student').style('color: #666; font-size: 11px;')
                            student_toggle = ui.switch('').props('color=red')
                        ui.button('Hinzufügen', on_click=add_person).props('no-caps unelevated').style(
