...
class Navbar:
    ...
    def render(self) -> None:
        """Rendert die fixe Navigationsleiste mit Login/Logout-Button."""
        ...
        with ui.header().classes(
            ...
            with ui.row().classes('items-center gap-6'):
                ...
                if authenticated:
                    ui.label('Gast' if is_guest else username).classes('text-gray-300 text-sm')
                    if is_admin:
                    ...
                    ui.button('Anmelden', on_click=lambda: ui.navigate.to('/login')).props(
                        'no-caps unelevated').classes(
                        'text-white text-sm font-bold rounded px-3 py-1'
                    ).style('background-color: #e50914 !important;')

    def _link_classes(self, page: str) -> str:
        """Gibt die CSS-Klassen für einen Nav-Link zurück (aktiv vs. inaktiv)."""
